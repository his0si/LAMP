...
baseline that uses INT8 budgets. Across Qwen2.5-1.5B-Instruct,
Gemma-2-2B-IT, and Qwen2.5-7B-Instruct, precision-aware scheduling
reduces per-token DRAM traffic by **1.78×–3.17×** at a memory operating
point of `B ≈ INT4_total / 2`, and enables fully on-chip resident
mixed-precision models at `B ≈ INT4_total`. We further show that the
sensitivity-driven mixed-precision policy commonly used as input — a
greedy per-layer assignment of Δppl per bit — survives joint GPTQ
quantization with Kendall τ = +1.0 against the predicted ranking, and
that promoting the score to a per-tile Δppl/Δbyte criterion produces
policies that protect attention KV projections (89 % of `v_proj`,
61 % of `k_proj`) at the same average bit-width budget.
...
spends >90 % of its cycles waiting for memory traffic [Yu et al.,
NeurIPS 2023 — TBD ref]. Reducing the per-token weight bytes is
...
that naive rounding loses. Recent LLM accelerator designs such as
GATHER [TBD ref] and the prior power-aware mapping work in MAGNETO
[TBD ref] take mixed-precision policies as system input and search the
dataflow / mapping space to extract energy efficiency on top.
...
- **Method extension.** Replacing the per-layer Δppl/Δbits greedy
  score with a per-tile Δppl/Δbyte score produces qualitatively
  different policies that promote attention KV projections (89 % of
  `v_proj` to INT8) at the same average bit budget; per-tile
  granularity is a prerequisite for HW-normalized scoring to differ
  from per-layer greedy.
...
SRAM banks (4 MB – 1 GB SRAM/HBM2-as-cache) [refs: Sanger, AntX, KEDA — TBD]
that hold a fraction of the model resident across token boundaries. This
paper focuses on that residency layer: given the policy and the buffer
size, which tiles stay.
...
### 6.4 E4 — Per-tile HW-normalized policy
...
**Residency consequence is modest.** At the same average bits per
weight, the two policies have similar total bytes (the policy
...
The win is small because the residency knapsack picks tiles by
absolute size — it does not care which logical role each tile plays.
We hypothesize the larger win is on the *accuracy* side: protecting
89 % of `v_proj` at 8-bit while keeping all `gate_proj` at 4-bit
should yield a lower ppl at iso-bpw than the per-layer policy. We do
not measure this directly in this draft (would require GPTQ
re-quantization for the hwnorm policies, ~50 min wall time per
policy); a follow-up evaluation is recorded in §7.2.
...
**Layer-wise mixed-precision policy search.** HAWQ [Dong 2019, Dong
2020], BRECQ [Li 2021], OmniQuant [Shao 2024], LLM.int8() [Dettmers
2022]. All take an algorithmic view; none address residency.

**LLM quantization at deployment.** GPTQ [Frantar 2023], AWQ [Lin
2023], SmoothQuant [Xiao 2023], Marlin [Frantar 2024]. Quantization
kernels, not scheduling.

**LLM accelerator dataflow.** GATHER, MAGNETO (lab-internal). Sanger
[Lu 2021], Ant [Guo 2022]. Mapping-side contributions, complementary
to ours.

**Memory residency / cache scheduling.** SARATHI [Agrawal 2024],
DistServe [Zhong 2024], FlexGen [Sheng 2023] — token-level
batching/swapping decisions for serving; coarser granularity than
per-tile pinning. ChunkAttention [Ye 2024] addresses KV-cache
residency, not weight residency.

**Knapsack-based scheduling in DNN inference.** TBD — typically
focuses on operator scheduling under throughput constraints, not
per-tile residency under capacity constraints.
...
scheduler reduces per-token DRAM traffic by **1.78×–3.17×** versus a
precision-oblivious INT8-budget baseline at a representative operating
point, and that the win generalizes across LLM family (Qwen vs Gemma)
and scale (1.5B–7B) with a factor that grows with model size. We
provide the full reproducibility trail in `experiments/`. The result
puts a name and a quantification on a layer of the stack between the
...
Qwen2.5-7B-Instruct 세 모델에 걸쳐, `B ≈ INT4_total/2` 운영점에서 토큰당
DRAM 트래픽이 **1.78×–3.17×** 감소하며, `B ≈ INT4_total` 영역에서는
혼합 정밀도 모델 전체가 on-chip 상주(fully on-chip resident)됩니다. 또한,
...
대기에 씁니다 [Yu et al., NeurIPS 2023 — 확인 필요]. 결국 토큰당
가중치 바이트를 줄이는 것이 지연 시간과 에너지 모두를 결정하는 가장
중요한 레버입니다.
...
복구합니다. 최근 LLM 가속기 설계 GATHER [TBD 인용]와 우리 연구실의
선행 power-aware mapping 연구인 MAGNETO [TBD 인용]는 혼합 정밀도 정책을
시스템 입력으로 받고 dataflow / mapping search 공간에서 에너지 효율을
끌어옵니다.
...
- **방법론 확장.** 레이어별 Δppl/Δbit 그리디 점수를 per-tile Δppl/Δbyte
  점수로 바꾸면, 동일 평균 비트 폭에서 어텐션 KV projection을 우선 보호하는
  (89 %의 `v_proj`이 8-bit) 질적으로 다른 정책이 만들어집니다. per-tile
  granularity가 HW-normalized 스코어와 per-layer 그리디가 차이를 만드는
  전제 조건입니다.
...
SRAM(4 MB ~ 1 GB SRAM 또는 HBM2 캐시) [Sanger, AntX, KEDA — 확인 필요]
입니다. 본 논문은 그 잔존 층에 초점을 둡니다: 정책과 버퍼 크기가
주어졌을 때 어떤 타일이 머무는가.
...
### 6.4 E4 — per-tile HW-normalized 정책
...
타일이 어떤 logical role인지는 모릅니다. 더 큰 이득은 *정확도* 쪽에서
기대됩니다. 89 %의 `v_proj`을 8-bit로 보호하고 `gate_proj`은 전부 4-bit로
두면 동일 bpw에서 per-layer 정책보다 낮은 ppl이 나와야 합니다. 본
드래프트에서는 직접 측정하지 않았습니다 (hwnorm 정책에 대한 GPTQ 재
양자화가 정책당 ~50분 wall-time 필요). §7.2에 후속 항목으로 기록.
...
**레이어별 혼합 정밀도 정책 탐색.** HAWQ [Dong 2019, 2020], BRECQ [Li 2021],
OmniQuant [Shao 2024], LLM.int8() [Dettmers 2022]. 모두 알고리즘 관점이며
잔존을 다루지 않습니다.

**LLM 양자화 배포.** GPTQ [Frantar 2023], AWQ [Lin 2023], SmoothQuant
[Xiao 2023], Marlin [Frantar 2024]. 양자화 커널, 스케줄링 아님.

**LLM 가속기 dataflow.** GATHER, MAGNETO (연구실 내). Sanger [Lu 2021],
Ant [Guo 2022]. 매핑 측 기여, 본 연구와 상보적.

**메모리 잔존 / 캐시 스케줄링.** SARATHI [Agrawal 2024], DistServe
[Zhong 2024], FlexGen [Sheng 2023] — 토큰 단위 batching/swapping 결정,
per-tile pinning보다 거친 granularity. ChunkAttention [Ye 2024]은 KV-
cache 잔존을 다루며, 가중치 잔존이 아닙니다.

**DNN 추론에서 knapsack 기반 스케줄링.** 추후 보강 — 보통 처리량 제약
하에서 연산자 스케줄링에 초점, 용량 제약 하의 per-tile 잔존이 아님.
...
예산 기준선 대비 토큰당 DRAM 트래픽을 대표 운영점에서 **1.78×–3.17×**
줄임을 보입니다. win은 LLM family (Qwen vs Gemma)와 scale (1.5B–7B)에
일반화되며, 그 *계수*는 모델 크기와 함께 커집니다. 전체 재현 trail은
`experiments/`에서 제공됩니다. 양자화 알고리즘과 dataflow 매퍼 사이의 한
층에 이름과 정량적 기여를 부여하며, 두 가지 자연스러운 후속을 가리킵니다:
prefetch-aware 스케줄링 모델, 그리고 동일 평균 비트 폭에서 어텐션 KV
projection을 보호하는 per-tile HW-normalized 정책.
